...
func main() {
	Router := gin.Default()
	// the cookie store is responsable for managing session data by storing in it encrypted cookie on the client side
	store := cookie.NewStore([]byte("Secret"))

	// this line line sets the middleware for the Router
	Router.Use(sessions.Sessions("session", store))

	// set fuction for find the path of the directory and Render the Html templates when requested
	Router.LoadHTMLGlob("Templates/*")
	// declearting the Error variable
	var ErrorMsg string
